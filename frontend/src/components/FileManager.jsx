...
const FileManager = () => {
    ...
    return (
        <div className="h-full w-full overflow-y-auto overflow-x-hidden scrollbar-thin scrollbar-thumb-white/10 relative" onClick={() => setContextMenu(null)}>
            <div className="p-6 flex flex-col gap-6">
                <input type="file" ref={fileInputRef} className="hidden" onChange={(e) => {/* handle upload if needed */ }} />

                {/* Toolbar */}
                <header className="liquid-card p-4 flex items-center justify-between z-10 shrink-0">
                    ...
                    <div className="flex items-center gap-2">
                        <button className="p-2.5 rounded-xl bg-gradient-to-r from-blue-600 to-cyan-500 text-white font-bold text-xs flex items-center gap-2 shadow-lg shadow-blue-500/20 hover:scale-105 transition-transform" onClick={() => {
                            const name = prompt('Enter folder name:');
                            if (name) handleCreate('folder');
                        }}>
                ...
                {/* Browser */}
                <div className="liquid-card flex flex-col p-6">
                    <div className="flex items-center justify-between mb-8">
                        <h3 className="text-xs font-black uppercase tracking-widest text-white/40">File Browser</h3>
                    </div>

                    <div className="relative">
                        {loading && <div className="absolute inset-0 flex items-center justify-center bg-black/40 backdrop-blur-sm z-20 rounded-2xl"><Loader2 className="animate-spin text-cyan-400" size={48} /></div>}
                        <div className="grid grid-cols-[repeat(auto-fill,minmax(140px,1fr))] gap-6">
                            {files.map(f => (
                                <motion.div
                                    ...
                                    className={`flex flex-col items-center p-6 rounded-2xl cursor-pointer transition-all duration-200 border group ${selection.includes(f.name)
                                ...
                                >
                                    <div className="mb-4 transition-transform duration-300 group-hover:scale-110 group-active:scale-95">
                                        {getIcon(f.name, f.isDirectory)}
                                    </div>
                                    <span className="text-sm font-bold text-white/90 text-center line-clamp-2 w-full break-all leading-tight">{f.name}</span>
                                    <span className="text-[10px] text-white/30 mt-2 font-mono uppercase tracking-widest">{f.size === 0 ? (f.isDirectory ? 'Directory' : '') : (f.size / 1024).toFixed(1) + ' KB'}</span>
                                </motion.div>
                            ))}
                        </div>
                    </div>
